...
var iom = iom || function () {
    var dummySite = "dummy", baseUrlDE = "de.ioam.de/tx.io", baseUrlLSO = "de.ioam.de/aid.io", optinUrl = "de.ioam.de/optin.php?re=", qdsUrl = "irqs.ioam.de", cntBaseUrl = ".iocnt.net/tx.io",
        cntBaseUrlLSO = ".iocnt.net/aid.io", cntOptinUrl = ".iocnt.net/optin.php?re=", cntQdsUrl = "irqs.iocnt.net", cntSubdomain = ["at", "imarex"],
        ...
        LSOBlacklist = [], checkEvents = 1, tb = 0, sv = 1, lastEvent = "", emptyCode = "Leercode_nichtzuordnungsfaehig", autoEvents = {onfocus: "aforg", onblur: "abakg", onclose: "aclse"}, nt = 2, cookiewhitelist = [],
        cookieName = "ioam2018", socioToken = "7b44ed5c17d7731e2cfb4edee2fc82e1", frequency = 6e4, hbiAdShort = 5e3, hbiAdMedium = 1e4, hbiAdLong = 3e4, hbiShort = 1e4, hbiMedium = 3e4, hbiLong = 6e4, hbiExtraLong = 3e5,
        heart;
    var IAMPageElement = null, IAMQSElement = null, qdsParameter = {}, qdsPopupBlockDuration = 864e5, result = {}, mode, eventsEnabled = 0, surveyCalled = 0, inited = 0;
    var lsottl = 864e5, lsottlmin = 18e4, ioplusurl = "me.ioam.de";
    ...
    function loadSurvey() {
        ...
        qdsParameter = {
            ...
            keysForQueryParam: ["offerIdentifier", "siteIdentifier", "sampleType", "pixelType", "isFadeoutFlash", "isFadeoutFrame", "isFadeoutForm", "positionTop", "positionLeft", "zIndex"]
        };
    ...
    function buildResult(params) {
        ...
        result.lo = "DE/Bayern";
        result.cb = "0016";
        result.i2 = "0016aeaf5098e002a5c17d773";
        result.ep = 1567522979;
        result.vr = "412";
    ...
    function forwardToOldSZM() {
        if (result.oer === "yes" && !window.IVW && !document.IVW) {
            ...
            (new Image).src = SZMProtocol + "//" + result.st + ".ivwbox.de/cgi-bin/ivw/" + SZMContType.toUpperCase() + "/" + SZMCode + ";" + SZMComment + "?r=" + escape(document.referrer) + "&d=" + Math.random() * 1e5
        }
    }
    ...
    function getFpcd(cd) {
        var ctld = "".match(/.{1,2}(?=(.{2})+(?!.))|.{1,2}$/g),
            blkPrefixes = ["www", "m", "mobile"], urlParts = cd.split("."), fpcd, ctldParts = [], hostParts = [], ctldPart = "", hostPart = "", i = 0, iLen = 0;
        if (!cd) return "";
    ...
    function setFirstPartyCookie(expire) {
        if (!expire) {
            expire = expireDays() * 24 * 60 * 60 * 1e3
        }
        var domain = getFpcd(location.hostname);
